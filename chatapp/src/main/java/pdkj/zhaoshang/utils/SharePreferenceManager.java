...
public class SharePreferenceManager {
    ...
    private static final String KEY_CACHED_USERTOKEN = "jchat_cached_usertoken";
    public static void setCachedUserToken(String token) {
        if (null != sp) {
            sp.edit().putString(KEY_CACHED_USERTOKEN, token).apply();
        }
    }

    public static String getCachedUserToken() {
        if (null != sp) {
            return sp.getString(KEY_CACHED_USERTOKEN, "");
        }
        return null;
    }
    ...
    public static void setAddress(String username) {
        if (null != sp) {
            sp.edit().putString("address", username).apply();
        }
    }
    ...
    private static final String HOMECONVERSATION_TOP = "homeconversation_top";
    public static void setHomeTopSize(int num) {
        if (null != sp) {
            sp.edit().putInt(HOMECONVERSATION_TOP, num).apply();
        }
    }

    public static int getHomeTopSize() {
        if (null != sp) {
            return sp.getInt(HOMECONVERSATION_TOP, 0);
        }
        return 0;
    }
